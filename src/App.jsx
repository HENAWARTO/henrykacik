...
const Portfolio = () => {
  ...
  const glassTilePattern = useMemo(() => ([
    { col: 2, row: 2, tone: 'from-amber-300/20 via-rose-400/10 to-cyan-300/20' },
    { col: 1, row: 2, tone: 'from-sky-300/20 via-fuchsia-300/10 to-amber-200/20' },
    { col: 2, row: 1, tone: 'from-violet-300/20 via-blue-300/10 to-emerald-200/20' },
    { col: 1, row: 1, tone: 'from-rose-300/20 via-orange-300/10 to-yellow-200/20' },
    { col: 2, row: 2, tone: 'from-emerald-300/20 via-cyan-300/10 to-blue-300/20' },
    { col: 1, row: 2, tone: 'from-indigo-300/20 via-purple-300/10 to-pink-200/20' },
    { col: 2, row: 1, tone: 'from-lime-300/20 via-teal-300/10 to-sky-200/20' },
  ]), []);
  ...
  useEffect(() => { setImgLoaded(false); setCopied(false); setHoveredPhoto(null); }, [idx, active]);
  useEffect(() => {
    if (active) {
        ...
        {active && (
          ...
          >
            <div className="max-w-6xl mx-auto">
              ...
              {!viewingImage ? (
                <motion.div initial={{opacity:0, y:12}} animate={{opacity:1, y:0}} transition={{duration:0.45}} className="pb-10">
                  <div className="relative overflow-hidden rounded-[2rem] border border-white/15 bg-[radial-gradient(circle_at_20%_10%,rgba(251,191,36,0.20),transparent_28%),radial-gradient(circle_at_80%_0%,rgba(56,189,248,0.16),transparent_30%),linear-gradient(135deg,rgba(24,24,27,0.96),rgba(0,0,0,0.98))] p-3 shadow-[0_35px_120px_rgba(0,0,0,0.65)] sm:p-5">
                    <div className="pointer-events-none absolute inset-0 opacity-50 [background-image:linear-gradient(115deg,transparent_0%,rgba(255,255,255,0.12)_48%,transparent_52%),radial-gradient(circle_at_center,transparent_45%,rgba(0,0,0,0.65)_100%)]" />
                    <div className="pointer-events-none absolute inset-3 rounded-[1.5rem] border border-white/10 shadow-[inset_0_0_40px_rgba(255,255,255,0.08)]" />
                    <div
                      className="relative grid auto-rows-[8rem] grid-cols-2 gap-2 sm:auto-rows-[10rem] sm:grid-cols-4 md:grid-cols-5 lg:auto-rows-[11rem]"
                      onMouseLeave={() => setHoveredPhoto(null)}
                    >
                      {active.photos.map((src, photoIdx) => {
                        const tile = glassTilePattern[photoIdx % glassTilePattern.length];
                        ...
                        return (
                          <button
                            ...
                            onFocus={() => setHoveredPhoto(photoIdx)}
                            onBlur={() => setHoveredPhoto(null)}
                            style={{
                              gridColumnEnd: `span ${isHovered ? Math.min(tile.col + 1, 3) : tile.col}`,
                              gridRowEnd: `span ${isHovered ? Math.min(tile.row + 1, 3) : tile.row}`,
                            }}
                            className="group relative min-h-0 overflow-hidden rounded-[1.35rem] bg-black text-left shadow-[0_18px_55px_rgba(0,0,0,0.42)] ring-2 ring-black/80 transition-[grid-column-end,grid-row-end,transform,filter] duration-500 ease-out hover:z-20 hover:-translate-y-1 hover:ring-white/50 focus:z-20 focus:outline-none focus:ring-2 focus:ring-white"
                            aria-label={`Open image ${photoIdx + 1} from ${active.title}`}
                          >
                            <img
                              src={src}
                              alt={active.captions?.[photoIdx] || `${active.title} production photo ${photoIdx + 1}`}
                              className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-110 group-focus:scale-110"
                              loading="lazy"
                              decoding="async"
                            />
                            <div className={`pointer-events-none absolute inset-0 mix-blend-screen bg-gradient-to-br ${tile.tone}`} />
                            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.28)_0%,transparent_18%,transparent_78%,rgba(255,255,255,0.12)_100%)] opacity-50" />
                            <div className="pointer-events-none absolute inset-0 rounded-[1.35rem] border border-white/25 shadow-[inset_0_0_22px_rgba(255,255,255,0.10),inset_0_0_60px_rgba(0,0,0,0.55)]" />
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/35 to-transparent p-3 text-white opacity-0 transition duration-300 group-hover:opacity-100 group-focus:opacity-100">
                              <div className="text-[10px] uppercase tracking-[0.24em] text-white/65">Photo {photoIdx + 1} of {active.photos.length}</div>
                              {active.captions?.[photoIdx] && <p className="mt-1 line-clamp-2 text-xs text-white/90">{active.captions[photoIdx]}</p>}
                            </div>
                          </button>
                        );
                      })}
